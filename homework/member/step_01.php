...
*이전 개인정보취급방침 보기(2014년 2월 1일부터 시행된 방침) 
			...
			<div class="all-agree-box">
				<label class="input-sp">
					<input type="checkbox" name="checkAll" id="checkbox03" value='no' />
					<span class="input-txt">상위 이용약관 및 개인정보 취급방침에 모두 동의합니다.</span>
				</label>
			</div>
			<div class="box-btn" >
                <form href="index.php?mode=step_02">
				<a id="next" class="btn-l" style="cursor:pointer">다음단계 (휴대폰인증)</a>
                </form>
			</div>
		</div>
	</div>
	...
	$(document).ready(function() {
		$("#next").click(function() {
			...
			if(!$("#checkbox02").is(":checked")) {
				alert("개인정보 취급방침에 동의를 체크해주세요.");
				return;
			}
			
			$.post("member_ajax.php", {mode:'registProgress', registProgress: $("input[name='checkAll']").val()}, function( data ) {
				alert(data.msg);
				if(data.result == "fail") {
					return;
				}
